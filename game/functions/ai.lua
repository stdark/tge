...
function ai.behavior()
	...
	trace.partyToDarkness(current_mob)--trace
	if helpers.mobCanSee (current_mob) then
		...
		for i=1,#chars_mobs_npcs do
			if (chars_mobs_npcs[i].party == chars_mobs_npcs[current_mob].party and helpers.mobCanSee (i))
			or chars_mobs_npcs[i].controlledby == chars_mobs_npcs[current_mob].ui then
				trace.first_watch (i);
				trace.one_around(i);
			end;
		end;
	...
	local agro_array = ai.agroArrayFull ();
...
--VOID	
	...
	if ai.behaviorCalled() then
		return;
	elseif ai.behaviorCruiser() then
		return;	
	elseif ai.behaviorShooter() then
		return;	
	elseif ai.behaviorBattleMage() then
		return;	
	elseif ai.behaviorHealer() then
		return;	
	elseif ai.behaviorBuffer() then
		return;	
	elseif ai.behaviorDebuffer() then
		return;	
	elseif ai.behaviorController() then
		return;	
	elseif ai.behaviorMelee() then
		return;	
	elseif ai.behaviorToEnemy(agro_array) then
		return;	
	elseif ai.behaviorToParty() then
		return;	
	elseif ai.behaviorRandomWalking() then
		return;	
	elseif ai.behaviorAway(agro_array) then
		return;	
	elseif ai.behaviorStay() then
		return;	
	elseif ai.behaviorToBase() then
		return;	
	end;
end;
...
function ai.mobWatchesTheMobNum (index,enemyonly) --for stealth
	...
	for i = 1, #chars_mobs_npcs do
		...
		then
			local value = 0;
			if not enemyonly or (enemyonly and not ai.friendOrFoe(i,index)) then
				value = 2 + chars_mobs_npcs[i].num_spothidden*chars_mobs_npcs[i].lvl_spothidden;
				if helpers.blindedWithLight (i,chars_mobs_npcs[i].x,chars_mobs_npcs[i].y) then
...
function ai.detectEnemiesInRange()
	...
	for j=1,#chars_mobs_npcs do
		...
		--and math.ceil(math.sqrt((chars_mobs_npcs[j].x - chars_mobs_npcs[current_mob].x)^2 + (chars_mobs_npcs[j].y - chars_mobs_npcs[current_mob].y)^2)) <= #shot_line
		and  #shot_line > 0
		and shot_line[#shot_line][1] == chars_mobs_npcs[j].x
		and shot_line[#shot_line][2] == chars_mobs_npcs[j].y 
		and chars_mobs_npcs[j].status == 1
		and darkness[chars_mobs_npcs[current_mob].party][chars_mobs_npcs[j].y][chars_mobs_npcs[j].x] == 0 
...
function ai.behaviorBattleMage()
...
function ai.behaviorDebuffer()
end;

function ai.behaviorController()
end;

function ai.behaviorAway(agro_array)
...
function ai.behaviorToEnemy(agro_array)
	if chars_mobs_npcs[current_mob].ai == "toenemy" then
		if not global.hang then
			local roll_point = 1;
			local free_hexes = helpers.findFreeHexes (current_mob);
			if #free_hexes > 0 then
				local distance = math.ceil(math.sqrt(free_hexes[1].x^2+free_hexes[1].y^2));
				...
				for j=1,#free_hexes do
					...
					and math.ceil(math.sqrt(free_hexes[j].x^2+free_hexes[j].y^2)) <= distance
					then
						distance = math.ceil(math.sqrt(free_hexes[j].x^2+free_hexes[j].y^2));
						roll_point = j;
					end;
				end;
				ai_world_x = free_hexes[roll_point].x;
...
function ai.behaviorRandomWalking()
